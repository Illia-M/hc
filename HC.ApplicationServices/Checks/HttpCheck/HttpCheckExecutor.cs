...
namespace HC.ApplicationServices.Checks.HttpCheck
{
    public class HttpCheckExecutor
    {
        ...
        public async Task CheckSite(Domain.HttpCheck.HttpCheck httpCheckSettings, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (httpCheckSettings.SuccessStatusCodes.Contains((ushort)result.StatusCode))
                {
                    ...
                    await _checkHistoryRepository.Store(new CheckStatus(httpCheckSettings.Uri!, true, sw.ElapsedMilliseconds, DateTimeOffset.UtcNow), cancellationToken);
                }
                else
                {
                    ...
                    await _checkHistoryRepository.Store(new CheckStatus(httpCheckSettings.Uri!, false, sw.ElapsedMilliseconds, DateTimeOffset.UtcNow), cancellationToken);
                }
            }
        ...
        private async Task TryNotify(Domain.HttpCheck.HttpCheck httpCheckSettings, string text, CancellationToken cancellationToken)
        {
            if (!string.IsNullOrEmpty(httpCheckSettings.TelegramChatId))
            {
